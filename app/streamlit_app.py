...
# Force dark theme + consistent colors
PRIMARY   = "#4f9fff"
SECONDARY = "#a78bfa"
SUCCESS   = "#34d399"
WARNING   = "#fbbf24"
...
.metric-label {{
    color: {TEXT_MUT}; font-size: 0.72rem; font-weight: 500;
...
.body-text {{ color: {TEXT_PRI}; font-size: 0.9rem; line-height: 1.8; }}
.muted-text {{ color: {TEXT_MUT}; font-size: 0.85rem; line-height: 1.7; }}
.step-title {{ color: {TEXT_PRI}; font-size: 0.85rem; font-weight: 600; margin: 0; }}
.step-desc  {{ color: {TEXT_MUT}; font-size: 0.78rem; margin: 0; }}

.nav-header {{
    color: {TEXT_MUT}; font-size: 0.68rem; font-weight: 600;
...
div[data-testid="stNumberInput"] label {{
    color: {TEXT_MUT} !important; font-size: 0.8rem !important;
}}
...
def pcfg(title="", h=300):
    """Base plotly layout — no yaxis conflict."""
    return dict(
...
with st.sidebar:
    ...
    for k, v in [("Algorithm","XGBoost + SMOTE"),("ROC-AUC","0.851"),
                 ("Dataset","Telco Churn"),("Records","7,043"),("Features","25")]:
        st.markdown(f"<p style='color:{TEXT_MUT};font-size:0.8rem;margin:0.25rem 0'>"
                    f"<span style='color:{TEXT_PRI};font-weight:500'>{k}:</span> {v}</p>",
                    unsafe_allow_html=True)
...
if page == "Project Overview":
    ...
    for col, label, val in [(c1,"Dataset Size","7,043"),(c2,"Features","25"),
                             (c3,"Churn Rate","26.5%"),(c4,"Best ROC-AUC","0.851")]:
    ...
    with col1:
        st.markdown(f'<div class="section-card"><p class="section-title">Problem Statement</p>'
                    f'<p class="body-text">Customer churn costs the telecom industry billions annually. '
                    f'Acquiring a new customer costs <strong style="color:{PRIMARY}">5–7x more</strong> than retaining one.</p>'
                    f'<p class="body-text" style="margin-top:0.8rem">This project builds a production-ready ML system that:</p>'
                    f'<ul style="color:{TEXT_PRI};font-size:0.9rem;line-height:2.2;margin-top:0.3rem">'
                    f'<li>Identifies at-risk customers before they leave</li>'
                    f'<li>Explains why a customer is at risk using SHAP</li>'
                    f'<li>Serves real-time predictions via a REST API</li>'
    ...
    with col2:
        steps = [
            ...
            ("06","Deployment","FastAPI REST API + Streamlit + Docker"),
        ]
        rows = "".join([f"""
        <div style='display:flex;gap:12px;align-items:flex-start;margin-bottom:12px'>
            <div style='min-width:32px;height:32px;border-radius:8px;background:{PRIMARY}22;
                 ...
                 color:{PRIMARY};font-size:0.7rem;font-weight:700;flex-shrink:0'>{num}</div>
            <div>
                <p class="step-title">{title}</p>
                <p class="step-desc">{desc}</p>
            </div>
        </div>""" for num,title,desc in steps])
        st.markdown(f'<div class="section-card"><p class="section-title">Project Pipeline</p>{rows}</div>',
                    unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with r1c1:
        fig = go.Figure(go.Pie(
            labels=["No Churn","Churn"], values=[73.5,26.5], hole=0.55,
    ...
    with r1c2:
        ...
        cfg = pcfg("Churn Rate by Contract Type")
        cfg["yaxis"]["title"] = "Churn Rate (%)"
    ...
    with r2c1:
        ...
        fig = go.Figure(go.Bar(
            x=["0–12 months","12–24 months","24–48 months","48–72 months"], y=vals,
            marker=dict(color=vals, colorscale=[[0,SUCCESS],[0.5,WARNING],[1,DANGER]]),
            text=[f"{v}%" for v in vals], textposition="outside",
            textfont=dict(color=TEXT_PRI)
        ))
        cfg = pcfg("Churn Rate by Tenure Group")
        cfg["yaxis"]["title"] = "Churn Rate (%)"
        fig.update_layout(**cfg)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with r2c2:
        fig = go.Figure(go.Bar(
            x=["Fiber optic","DSL","No Internet"], y=[41.9,19.0,7.4],
            marker_color=[DANGER,WARNING,SUCCESS],
            text=["41.9%","19.0%","7.4%"], textposition="outside",
            textfont=dict(color=TEXT_PRI)
        ))
        cfg = pcfg("Churn Rate by Internet Service")
        cfg["yaxis"]["title"] = "Churn Rate (%)"
        fig.update_layout(**cfg)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with r3c1:
        ...
        fig = go.Figure(go.Bar(
            x=["Electronic check","Mailed check","Bank transfer","Credit card"], y=vals,
            marker=dict(color=vals, colorscale=[[0,SUCCESS],[0.5,WARNING],[1,DANGER]]),
            text=[f"{v}%" for v in vals], textposition="outside",
            textfont=dict(color=TEXT_PRI)
        ))
        cfg = pcfg("Churn Rate by Payment Method")
        cfg["yaxis"]["title"] = "Churn Rate (%)"
        fig.update_layout(**cfg)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with r3c2:
        fig = go.Figure(go.Bar(
            x=["No Security & No Support","Has One Service","Has Both"], y=[51.2,28.4,14.7],
            marker_color=[DANGER,WARNING,SUCCESS],
            text=["51.2%","28.4%","14.7%"], textposition="outside",
            textfont=dict(color=TEXT_PRI)
        ))
        cfg = pcfg("Churn Rate by Support Services")
        cfg["yaxis"]["title"] = "Churn Rate (%)"
        fig.update_layout(**cfg)
        st.plotly_chart(fig, use_container_width=True)

    st.markdown(f"<p style='color:{PRIMARY};font-size:0.78rem;font-weight:600;"
                f"text-transform:uppercase;letter-spacing:0.1em;margin-top:0.5rem'>"
                f"Feature Correlation with Churn</p>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="hero-banner">
        ...
        <p class="hero-subtitle">Enter customer details to get a real-time churn risk assessment.</p>
    </div>""", unsafe_allow_html=True)
    ...
    if predict_clicked:
        ...
        try:
            ...
            st.markdown(f"<p style='color:{PRIMARY};font-size:0.78rem;font-weight:600;"
                        ...
                        f"Key Risk Factors</p>", unsafe_allow_html=True)
            factors = []
            ...
            if internet=="Fiber optic" and monthly>70: factors.append(("High Charges",f"${monthly}/mo on Fiber optic is above the average spend.",WARNING))
            ...
            if tenure>36: factors.append(("Loyal Customer",f"{tenure} months of tenure — long-term loyalty reduces churn.",SUCCESS))
            ...
            st.markdown(f"<p style='color:{PRIMARY};font-size:0.78rem;font-weight:600;"
                        f"text-transform:uppercase;letter-spacing:0.1em;margin-top:1rem'>"
                        f"Customer Summary</p>", unsafe_allow_html=True)
            ...
            for col,label,val in [
                (m1,"Tenure",f"{tenure} mo"),(m2,"Monthly Charges",f"${monthly:.0f}"),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for name,formula,explanation,feat_type,importance in features:
        ...
        <div style='background:{BG_CARD};border:1px solid {BORDER};border-left:4px solid {PRIMARY};
            ...
            <div style='display:flex;justify-content:space-between;align-items:flex-start;
                ...
                <p style='color:{TEXT_PRI};font-size:1rem;font-weight:700;margin:0;
                   font-family:monospace'>{name}</p>
    ...
    st.markdown(f"<p style='color:{PRIMARY};font-size:0.78rem;font-weight:600;"
                f"text-transform:uppercase;letter-spacing:0.1em;margin-top:0.5rem'>"
                f"Feature Impact on Churn Rate</p>", unsafe_allow_html=True)
    feat_names = ["is_monthly","no_support","risky_payment","charges_per_tenure","num_addons","tenure_group"]
    high_risk  = [42.7,51.2,45.3,38.1,12.4,47.7]
    low_risk   = [2.8,14.7,18.2,18.9,38.6,6.6]
    fig = go.Figure()
    fig.add_trace(go.Bar(name="High Risk Group", x=feat_names, y=high_risk, marker_color=DANGER))
    fig.add_trace(go.Bar(name="Low Risk Group",  x=feat_names, y=low_risk,  marker_color=SUCCESS))
    layout = pcfg(h=320)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    m1,m2,m3,m4 = st.columns(4)
    for col,label,val in [(m1,"Best ROC-AUC","0.851"),(m2,"Best F1","0.623"),
    ...
    df = pd.DataFrame({
        ...
        "ROC-AUC":[0.8426,0.8381,0.8370,0.8510],
        "F1 Score":[0.6227,0.5923,0.5884,0.6140],
        "Precision":[0.5168,0.5884,0.5807,0.6020],
        "Recall":[0.7834,0.5963,0.5963,0.6280],
    })
    ...
    with col2:
        ...
        layout = pcfg("Model Metrics Comparison", h=360)
        layout["barmode"] = "group"
    ...
    # SHAP
    st.markdown(f"<p style='color:{PRIMARY};font-size:0.78rem;font-weight:600;"
                f"text-transform:uppercase;letter-spacing:0.1em'>SHAP Feature Importance — Top 10</p>",
                unsafe_allow_html=True)
    ...
    fig = go.Figure(go.Bar(
        ...
        text=[f"{v:.3f}" for v in shap_vals], textposition="outside",
        textfont=dict(color=TEXT_PRI)
    ))
    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor=BG_CARD,
    ...
    # Confusion matrix
    st.markdown(f"<p style='color:{PRIMARY};font-size:0.78rem;font-weight:600;"
                f"text-transform:uppercase;letter-spacing:0.1em'>Confusion Matrix — XGBoost (Tuned)</p>",
                unsafe_allow_html=True)
    fig = go.Figure(go.Heatmap(
        z=[[874,178],[148,209]], x=["No Churn","Churn"], y=["No Churn","Churn"],
        colorscale=[[0,BG_CARD],[1,PRIMARY]],
        text=[["874","178"],["148","209"]],
        texttemplate="%{text}", textfont={"size":22,"color":TEXT_HEAD}
    ))
    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor=BG_CARD,
